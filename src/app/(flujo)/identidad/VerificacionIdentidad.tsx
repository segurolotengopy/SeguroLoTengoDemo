...
const TARJETAS: readonly {
...
}[] = [
  {
    ...
    detalle: "Documento completo, enfocado y sin reflejos.",
    boton: "Tocá acá para fotografiar el frente",
  },
  {
    tipo: "DORSO",
    numero: 2,
    ...
    boton: "Tocá acá para fotografiar el dorso",
  },
  {
    tipo: "SELFIE",
    numero: 3,
    ...
    // Por eso este rótulo no dice «en vivo» ni habla de prueba de vida: sin
    // liveness, decirlo sería afirmar algo que no ocurrió.
    detalle: "Tu rostro dentro del óvalo, de frente y con buena luz.",
    boton: "Tocá acá para tomar tu selfie",
  },
];
...
const TARJETA_SELFIE_EN_VIVO = {
  ...
  detalle: "Seguí los movimientos de la prueba de vida.",
  boton: "Tocá acá para iniciar la verificación",
} as const;
...
export function VerificacionIdentidad({
  ...
  /** Las tres capturas tomadas y aprobadas: recién ahí se puede leer la cédula. */
  const capturasCompletas = TARJETAS.every(({ tipo }) => capturas[tipo]?.aprobada === true);
      ...
      <div className="flex flex-col gap-3">
        ...
        {datos === null ? (
          <div className="flex flex-wrap items-center gap-3">
            <button
              type="button"
              data-cta="Acá abajo está el botón para leer tu cédula"
              onClick={() => void analizar(capturas)}
              disabled={!capturasCompletas || enProceso !== null}
              className="btn btn-primary"
              style={{ borderRadius: "999px", padding: "13px 22px", fontSize: "15px" }}
            >
              {enProceso === "ANALISIS"
                ? "Leyendo tu cédula…"
                : "Tocá acá para leer los datos de mi cédula →"}
            </button>
            <span className="text-sm text-etiqueta">
              {enProceso === "ANALISIS"
                ? "Estamos extrayendo nombre, número y fecha de nacimiento."
                : capturasCompletas
                  ? "Tarda unos segundos."
