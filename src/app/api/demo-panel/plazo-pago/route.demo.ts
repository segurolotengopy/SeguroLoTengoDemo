/**
 * `POST /api/demo-panel/plazo-pago` — acelerador del plazo para firmar.
 *
 ...
 * `src/adapters/mock/plazo-pago-demo.ts`. Acá solo está la puerta.
 *
...
import { fijarPlazoPagoDemo, plazoPagoMs } from "@/adapters/mock/plazo-pago-demo";
...
export async function POST(request: Request): Promise<Response> {
  ...
  const resultado = fijarPlazoPagoDemo(ms);
  if (!resultado.ok) {
    return respuestaJson({ ok: false, motivo: resultado.motivo }, { status: 400 });
  }

  return respuestaJson({ ok: true, plazoMs: plazoPagoMs() });
}
