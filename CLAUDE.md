...
- **Nunca cites de memoria un artículo de ley.** Si necesitás justificar una regla de negocio, buscá la fila correspondiente en `docs/Tabla Cumplimiento SeguroLo Tengo - Tabla.csv` y citá Número + Categoría + Norma y Artículo tal como figuran ahí. **Y contrastá la cita contra `docs/MATRIZ_LEGAL_V4.md` y `docs/normativa/`**: el CSV tiene errores de cita comprobados, y la V4 es posterior y prevalente.
- **Las dos normas centrales del canal electrónico son la Res. SS.SG. 210/2025 y la 231/2025**, ambas en `docs/normativa/` y leídas de primera mano. El CSV no las menciona. La 210/2025 art. 4 es el respaldo de la firma simple del cliente y su art. 5 vuelve obligatoria la firma cualificada del corredor.
