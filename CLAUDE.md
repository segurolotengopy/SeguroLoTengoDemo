...
| `Integraciones/eCommerce_bancard_compra_simple_version_1.23.1 (1).pdf`, `Integraciones/Preaut y promociones 14.pdf`, `Integraciones/Qr en API de Comercios v1.2 16 (1).pdf` | Contrato técnico exacto de las APIs de Bancard: compra simple de eCommerce, preautorización y captura, y QR de comercios. Gobiernan el adaptador oficial de `PaymentProvider` (P7) **y también el mock** (decisión de Andres del 21-ago-2026): los mensajes y el comportamiento simulados salen de estos documentos, no se inventan. De acá vienen los `response_code` (`00`, `05`, `12`, `15`, `51`) con su descripción, el `qr_data` en formato EMVCo, el `hook_alias` y la regla de que Bancard aguarda 5 s el callback y reversa si no hay respuesta. Mismo criterio que con Code100: no inventar parámetros ni endpoints.                                                                                              |
