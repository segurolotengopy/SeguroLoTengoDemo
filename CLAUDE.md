...
| `Integraciones/Bancard - Respuestas B1 a B13.md` | **Respuestas del proveedor a las trece consultas** (27-ago-2026 el equipo técnico; 28-ago-2026 B2 y B3, el comercial), con su análisis en `docs/ANALISIS_RESPUESTAS_BANCARD.md`. Lo que manda: el **QR dinámico vive 3 días** (más que el plazo de pago de 24 h); **no existe** la consulta de pagos por `hook_alias` —el callback es la única fuente de verdad del estado de un pago QR—; el `shop_process_id` es de **un solo uso de por vida**, aun si el intento falló; los callbacks **no van firmados** (Basic + HTTPS + whitelist de IP, 5 s para QR y 30 s para vPOS); y el bloqueo antifraude es **por tarjeta, no por tarjeta+comercio**, no consultable por el comercio (recomiendan ≤ 5 intentos/24 h). Y sobre devoluciones: la devolución posterior al cuponado **no genera constancia documental** —el único respaldo es el estado en el Portal de Comercios—, vuelve al mismo plástico o a la cuenta según el medio, con SLA de 48-72 h **solo** para crédito. Leerlo antes de tocar `PaymentProvider` o los textos de devolución. Siguen pendientes **B7** y **B11**. |
