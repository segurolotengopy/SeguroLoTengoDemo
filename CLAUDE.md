...
| `Integraciones/Bancard - Respuestas B1 a B13 (parte 1).md` | **Respuestas del proveedor a las trece consultas técnicas** (27-ago-2026), con su análisis en `docs/ANALISIS_RESPUESTAS_BANCARD.md`. Lo que manda: el **QR dinámico vive 3 días** (más que el plazo de pago de 24 h); **no existe** la consulta de pagos por `hook_alias` —el callback es la única fuente de verdad del estado de un pago QR—; el `shop_process_id` es de **un solo uso de por vida**, aun si el intento falló; los callbacks **no van firmados** (Basic + HTTPS + whitelist de IP, 5 s para QR y 30 s para vPOS); y el bloqueo antifraude es **por tarjeta, no por tarjeta+comercio**, no consultable por el comercio (recomiendan ≤ 5 intentos/24 h). Leerlo antes de tocar `PaymentProvider`. B2, B3, B7 y B11 siguen pendientes. |
