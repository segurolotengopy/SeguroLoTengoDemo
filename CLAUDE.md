...
**Dos capas conviven hoy, y hay que saber cuál se está mirando:**

1. **Lo que corre en producción** (`FLUJO_V4` apagado): el flujo de 8 pasos
   de v2 con la **base visual v4 aplicada encima** (PR #126: paleta,
   `HeaderInstitucional` de tres marcas, `StepperPasos` de 5 etapas por
   `etapaDePaso(slug)`, Arimo, tema claro) y la pantalla 02 rehecha en su
   lugar. La batería E2E de la raíz `e2e/` prueba **esta** capa.
2. **Las doce pantallas v4** (PR #131), detrás de `FLUJO_V4=true`: cada
   `page.tsx` del flujo elige entre su cuerpo v2 y `src/components/v4/pantallas/`
   según `flujoV4Activo()`. Marco propio (`MarcoV4` = `CabeceraV4` con menú
   01B + `StepperV4` por **código de pantalla** de `src/domain/v4/etapas.ts`),
   `src/app/v4.css` bajo `[data-flujo="v4"]`, cámara con `piel="v4"`.
   Encenderla para siempre —unificar el marco con `HeaderInstitucional`,
   borrar los cuerpos v2/v3 y reescribir los E2E contra estas pantallas— es
   el PR siguiente.
...
Lo que v4 cambia y **ya está implementado** (en las dos capas, salvo que se
diga lo contrario):
...
- **Carga de archivo del frente y el dorso en producción** (D-46): con
  `FLUJO_V4` ya no depende de `DEMO_MODE` (`origenCapturaAdmitido`). La
...
- **Doce pantallas** (capa 2) y sus rutas: `/` (01) · `/plan` (02) ·
...
- **Cinco endpoints nuevos**, todos bajo `/api/v4/`: `identidad`,
  `datos-personales`, `actividad`, `declaraciones` y `consentimientos`. Y las
  rutas de la firma interna (`/api/p8/firma-interna/*`) responden también con
  `FLUJO_V4`, no solo con `FLUJO_V3`.
...
| Archivo                                                                                                                                                                     | Úsalo para...                                                                                                                                                                                                                                                                                                                                                |
| :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BITACORA.md`                                                                                                                                                               | **Leela al empezar cualquier sesión.** Registro cronológico de qué se cambió, qué decidió o ejecutó Andres, con qué se verificó y qué quedó abierto. Es donde vive el *caso* de cada cambio — el porqué, lo que se descartó y con qué prueba— que el resto del repo no conserva. La sección `Queda abierto` de la entrada más reciente es lo primero que hay que mirar. Se agrega una entrada por sesión, arriba. |
...
| `Integraciones/eCommerce_bancard_compra_simple_version_1.23.1 (1).pdf` y `Integraciones/Qr en API de Comercios v1.2 16 (1).pdf` | Contrato técnico exacto de las dos APIs de Bancard que usa el producto: compra simple de eCommerce (las dos tarjetas) y QR de comercios. Gobiernan el adaptador oficial de `PaymentProvider` (P7) **y también el mock** (decisión de Andres del 21-ago-2026): los mensajes y el comportamiento simulados salen de estos documentos, no se inventan. De acá vienen los `response_code` (`00`, `05`, `12`, `15`, `51`) con su descripción, el `qr_data` en formato EMVCo, el `hook_alias` y la regla de que Bancard aguarda 5 s el callback y reversa si no hay respuesta. Mismo criterio que con Code100: no inventar parámetros ni endpoints.                                                                                              |
| `Integraciones/Preaut y promociones 14.pdf` | Preautorización y captura de Bancard. **No gobierna ningún adaptador**: la preautorización se quitó del puerto por D-02, porque con el orden invertido de D-08 el cobro ocurre después de la firma y no hay nada que reservar. Se conserva como documentación del proveedor para el día que el flujo la necesite; si ves código o texto que la dé por vigente, está sin migrar. |
| `Integraciones/Bancard - Respuestas B1 a B13.md` y `Integraciones/Bancard - Respuestas segunda ronda.md` | **Respuestas del proveedor**, primera ronda (27-ago-2026 el equipo técnico; 28-ago-2026 B2 y B3, el comercial) y segunda (07-sep-2026), con su análisis en `docs/ANALISIS_RESPUESTAS_BANCARD.md`. Lo que manda: el **QR dinámico vive 3 días** y **no es configurable**, pero la **reversa por `hook_alias` invalida un QR generado y no pagado** —Bancard misma recomienda usarla para apagarlo a las 24 h—; **no existe** la consulta de pagos por `hook_alias` y el callback es la única fuente en vivo del estado de un pago QR, sin reintentos y con **reversa automática ante timeout**; el `shop_process_id` es de **un solo uso de por vida**, y un intento **rechazado sí llega al callback y sí lo devuelve `get_confirmation`**; los callbacks **no van firmados** (Basic + HTTPS + whitelist de IP, 5 s para QR y 30 s para vPOS); y el bloqueo antifraude es **por tarjeta, no por tarjeta+comercio**, no consultable por el comercio (recomiendan ≤ 5 intentos/24 h). Sobre devoluciones: la posterior al cuponado **no genera constancia documental** —el único respaldo es el estado en el Portal de Comercios—, vuelve al mismo plástico o a la cuenta según el medio, con SLA de 48-72 h **solo** para crédito. Leerlo antes de tocar `PaymentProvider` o los textos de devolución. Siguen pendientes **B7**, **B13-bis**, **B11** y **B3-bis**. |
...
| `Tabla Cumplimiento SeguroLo Tengo - Tabla.csv`                                                                                                                             | Matriz normativa (Número, Categoría R1–R8, Título, Norma y Artículo). Fuente de verdad regulatoria del proyecto — ver "Regla de trabajo con los documentos" abajo. 93 filas; las 86-93 son la Res. 210/2025.                                                                                                                                                 |
| `CRITERIO_UNIFICADO_NORMATIVA_Y_SEGURIDAD.md`                                                                                                                               | **Criterio operativo unificado** (26-ago-2026): qué norma rige cada tema, la decisión de arquitectura de firma (FENC propia del cliente), y el mapa requisito normativo → mecanismo de seguridad AWS por horizonte (demo / L6 / D1 / Go-Live / trámites externos). Leelo antes de tocar firma, evidencia, conservación o infraestructura.                     |
| `normativa/INDICE.md`                                                                                                                                                       | Qué textos oficiales están en `docs/normativa/` y cuáles faltan (con su dirección oficial), más la lista de normas derogadas que no deben citarse. Consultalo **antes** de citar una norma.                                                                                                                                                                  |
| `auditoria/ANALISIS_RES_210_2025.md`                                                                                                                                        | Análisis artículo por artículo de la Res. 210/2025 contra el sistema, con los tres documentos del 26-ago-2026 cruzados y el estado de cada acción.                                                                                                                                                                                                           |
...
**Única excepción, y solo con `DEMO_MODE=true`:** las tres capturas —frente, dorso y **selfie**— se pueden subir como archivo (decisión de Andres del 20-ago-2026, implementada el 21). Que la selfie entre no es que sea inocua: es el ancla biométrica, y un archivo ahí permite verificar la identidad con la cara de otra persona. Entra igual porque este camino **ya renunció a la prueba de vida** —`decidirPresenciaDemo` comprueba presencia, no vida, y una foto impresa frente a la cámara pasaría—, así que exigir la cámara para la selfie no compraba la garantía que parecía comprar, y a cambio cortaba toda demostración a distancia en la que la persona no está frente al equipo. Lo decide `origenCapturaAdmitido()` en el dominio, y **quien lo hace cumplir es el Route Handler**, no la pantalla: esconder el botón es cosmético, cualquiera arma la petición a mano. El origen (`CAMARA` / `ARCHIVO`) va a la evidencia, así que un expediente con documento o rostro subidos nunca queda registrado como fotografiado en vivo. Es comodidad de demostración, sin fila en la matriz de cumplimiento; producción sigue exigiendo cámara y Face Liveness.
...
- **El sexo no lo completa el OCR**: se elige, entre los dos valores que puede decir una cédula (21-ago-2026). Era el campo donde el prellenado rendía menos —son dos opciones, elegir cuesta un toque— y costaba más, porque una lectura errada pasaba inadvertida justo por venir ya puesta, hasta aparecer en el documento firmado. Viaja igual en `correcciones.sexo` y el servidor lo sigue cotejando contra `SEXOS_ADMITIDOS`.
