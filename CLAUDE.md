...
- **Escritorio: los componentes van lado a lado, sin cambiar orden, textos ni jerarquía (D-30, implementada el 17-sep-2026).** Los artes son de celular; la disposición de escritorio de cada pantalla sale **solo** de `src/components/v4/disposicion.tsx`: `DisposicionV4` (columna de contexto —`EncabezadoV4` + `BarraPlanV4`— fija a la izquierda, columna de acción a la derecha, desde `lg`), `RejillaV4` (listas de tarjetas iguales en 2–3 columnas), `CamposV4`/`CampoAnchoV4` (campos cortos y hermanos lado a lado, los largos a fila entera), `AccionesV4` (el botón con ancho de botón, alineado al final de la lectura) y `ProsaV4` (~65 caracteres por línea). `MarcoV4` da el lienzo de 72 rem. Toda clase de escritorio lleva prefijo (`sm:`/`md:`/`lg:`); **el DOM conserva el orden del arte** —celular, lector de pantalla y batería E2E ven lo mismo— y ninguna pantalla inventa su propia rejilla. Las hojas (`HojaV4`) se centran como diálogo en escritorio. Se aprueba con las capturas del PDF de gerencia (1456 px y 390 px).
