...
4. **Refactor interno — ~~pendiente~~ HECHO (D-02).** ~~Quitar `iniciarPreautorizacionTarjeta` y `capturarPreautorizacion` de `src/ports/payment-provider.ts` (y del mock, sus tests de contrato y la palanca "fallo de captura de Bancard" del panel de demo), y actualizar los documentos que describen crédito = preautorización.~~

   **Cerrado el 2026-09-05.** Lo ejecutó la decisión **D-02** (`docs/plan/DECISIONES.md`) junto con **D-08**, que además invirtió el orden: el cobro ocurre **después** de la firma, no antes, así que el punto 2 de arriba —redactado cuando el cobro precedía a la firma— quedó por detrás de lo vigente. Estado actual: el puerto expone `iniciarPagoQr`, `iniciarPagoTarjetaDebito` y `iniciarPagoTarjetaCredito`, sin reserva ni captura; la palanca `BANCARD_CAPTURA_FALLIDA` se retiró; `ESPECIFICACION_PANTALLAS.md` y `CLAUDE.md` ya no describen preautorización.
