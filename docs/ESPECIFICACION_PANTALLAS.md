...
- `ASEGURADORA · Alianza Garantía Seguros y Reaseguros S.A.` con logo y sus referencias regulatorias.
- `INTERMEDIARIO · Interseguros S.A. · Corredores de Seguros` con logo y matrícula.
- `SLT CANAL DIGITAL · SeguroLoTengo.com — Marca de Interseguros S.A.`
...
**Pie legal (todas las pantallas):** bloque expandible `INFORMACIÓN LEGAL Y
REGULATORIA` con el texto institucional (canal, aseguradora, producto, firma
electrónica del cliente simple autenticada por código de un solo uso, firmas
institucionales cualificadas, pagos por Bancard a favor de la aseguradora,
cobertura 24 h después del pago confirmado) y **siete enlaces**: Términos y
condiciones · Aviso de privacidad · Coberturas, exclusiones y carencias ·
Condiciones generales · Consultas y reclamos · Derecho de retracto ·
Verificación de documentos. Los enlaces abren el **modal de aclaraciones**
(`AclaracionModal`) sin abandonar la pantalla; `/retracto` y
`/verificar/<código>` conservan además su página propia.
...
**Paleta y tipografía:** las de `docs/GUIA_DE_ESTILOS.md` (DM Sans, tokens
semánticos, acentos naranja/verde/rojo), como hasta ahora.
