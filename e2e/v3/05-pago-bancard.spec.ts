/**
 * Flujo v3 · La parte de Bancard en la pantalla que la demo muestra.
 *
 * El paso 3 monta el mismo `FormularioPagoP7` que v2, así que lo que se prueba
 * acá vale para los dos; lo que **no** se puede probar en otro lado es lo que
 * depende de la pantalla, y es exactamente lo que este archivo cubre:
 *
 * - **G2** · que un rechazo de Bancard no deje a la persona encerrada. Mientras
 *   hay una operación abierta, la pantalla bloquea el botón y el cambio de
 *   medio; un rechazo que solo cortara el sondeo dejaría un error correcto y
 *   nada que hacer con él. El test verifica que el botón vuelva, que el mensaje
 *   diga la razón del proveedor, y que **el reintento cobre**.
 *
 ...
 *
 * **G1 (la reversa al vencer) no se prueba acá a propósito:** exige adelantar
 * el plazo de pago y esperar el vencimiento dentro del recorrido completo, que
 * son varios minutos por una garantía que el test de integración ya verifica
 * mirando la operación del lado de Bancard. El E2E no agregaría nada que la
 * pantalla muestre.
 */
import { expect, test } from "@playwright/test";
import { llegarAPagoYFirmaFirmado } from "./soporte/llegar-a-firmado";
...
test("una tarjeta rechazada no encierra a la persona: puede reintentar y cobrar", async ({
  ...
  await llegarAPagoYFirmaFirmado(page, { fallas: ["BANCARD_TARJETA_RECHAZADA"] });

  // ── Primer intento: la operación se abre bien ───────────────────────────
  // Con débito, porque es el medio donde el rechazo de tarjeta ocurre de
  // verdad: la persona tipea sus datos en el formulario seguro de Bancard y el
  // emisor contesta que no.
  ...
  const pagar = page.getByRole("button", { name: "PAGAR CON DÉBITO" });
